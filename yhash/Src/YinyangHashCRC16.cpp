...
DWORD YinyangHashCRC16Buffer(const void* pBuffer, DWORD crc, DWORD len);

extern "C" const DWORD crc16tbl[256] = {
...
YinyangHashCRC16::YinyangHashCRC16()
{
    Initialize();
}
...
void YinyangHashCRC16::Initialize()
{
    YinyangHashBase::Initialize();
    m_dwCRC16 = 0xffff;
}

void YinyangHashCRC16::ComputeHashFinalHelper(void *msg_digest)
{
    WORD *pBuf = (WORD*)msg_digest;
    DWORD dwCRC = m_dwCRC16;

    if ( m_dwRestLen > 0 )
        dwCRC = YinyangHashCRC16Buffer(m_pRest, dwCRC, m_dwRestLen);

    *pBuf = (WORD)((dwCRC<<8) | (dwCRC>>8));
}

DWORD YinyangHashCRC16::ComputeHashHelper(const void *pBuffer, void *msg_digest, DWORD len)
{
    m_dwCRC16 = YinyangHashCRC16Buffer(pBuffer, m_dwCRC16, len & 0xFFFFFFC0);
    return len % 64;
}
...
DWORD YinyangHashCRC16Buffer(const void *pBuffer, DWORD crc, DWORD len)
{
    ...
    for (i=0; i<len; i++)
        crc = crc16tbl[(crc ^ p[i]) & 0xff] ^ (crc >> 8);
    return crc;
}